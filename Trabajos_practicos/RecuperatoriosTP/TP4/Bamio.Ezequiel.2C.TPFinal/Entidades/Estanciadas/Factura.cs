...
namespace Entidades
{
    public class Factura
    {
        ...
        public string Recibo() 
        {
            ...
            if (this.Productos.Count > 0)
            {
                ...
                foreach (Producto producto in this.Productos)
                {
                    ...
                    factura.AppendLine($"Precio c/u: {producto.Precio}");
                    factura.AppendLine($"Cantidad: {producto.Stock}");
                    factura.AppendLine("-----------------------------");
                }
        ...
        public static bool ValidarClienteFactura(List<Factura> facturas, int idCliente) 
        {
            foreach (Factura f in facturas)
            {
                if (f.IdCliente == idCliente) 
                {
                    return true;
                }
            }
